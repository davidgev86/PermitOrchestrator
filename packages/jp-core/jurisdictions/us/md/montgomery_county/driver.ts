...
export const driver: PortalDriver = {
  async submit(pkg: SubmissionPackage): Promise<{ portalCaseId: string; receiptUrl?: string }> {
    console.log(`Submitting permit package for case ${pkg.caseId} to Montgomery County DPS portal`);
    
    const portalCaseId = `MC-DPS-${Date.now()}-${pkg.caseId.slice(-6)}`;
  ...
  async pollStatus(portalCaseId: string): Promise<PortalStatus> {
    ...
    const statuses = ["Application Received", "Plan Review in Progress", "Corrections Required", "Approved", "Permit Issued"];
  ...
  async requestInspection(
    ...
    const confirmationId = `MC-INSP-${Date.now()}`;
